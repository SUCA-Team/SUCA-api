from root_fsrs import Scheduler, Rating, State, Card
...
class MyCard:
    ...
    def __init__(self, front: str, back: str, card_id: int | None = None, fsrs_card: Card | None = None):
    ...
    @classmethod
    def from_dict(cls, data: dict) -> 'MyCard':
        """Create MyCard from dict"""
        fsrs_card: Card = Card.from_json(data["card_json"])
        return cls(
            front=data["front"],
            back=data["back"],
            card_id=data["card_id"],
            fsrs_card=fsrs_card5
        )

# ==========================
# STORAGE
# ==========================
DATA_FILE: Path = Path(__file__).parent / "flashcards_data.json"
...
def load_user_cards(db: dict, username: str) -> list[MyCard]:
    """Load cards for a user"""
    if username not in db["users"]:
        return []
    
    cards_data: list = db["users"][username].get("cards", [])
    return [MyCard.from_dict(c) for c in cards_data]

def save_user_cards(db: dict, username: str, cards: list[MyCard]) -> None:
...
def is_card_new(card: Card) -> bool:
    ...
    return (card.stability is None and 
            card.difficulty is None and 
            card.last_review is None)

def get_due_cards(cards: list[MyCard]) -> list[MyCard]:
    """Get list of due cards"""
    now: datetime = datetime.now(timezone.utc)
    
    learning: list[MyCard] = []
    review: list[MyCard] = []
    new: list[MyCard] = []
    
    for my_card in cards:
        fsrs_card: Card = my_card.fsrs_card
...
# ADD CARD
# ==========================
def add_card(db: dict, username: str, cards: list[MyCard]) -> None:
    """Add new card"""
    print("\n" + "="*50)
    print("ADD NEW CARD")
    print("="*50)
    ...
    new_card: MyCard = MyCard(front, back)
...
# REVIEW SESSION
# ==========================
def review_cards(db: dict, username: str, cards: list[MyCard], scheduler: Scheduler) -> None:
    """Review session"""
    due_cards: list[MyCard] = get_due_cards(cards)
    
    if not due_cards:
        print("\nNo cards due now!")
        
        # Show next due time
        active_cards: list[MyCard] = [c for c in cards if not is_card_new(c.fsrs_card)]
        if active_cards:
            next_card: MyCard = min(active_cards, key=lambda c: c.fsrs_card.due)
            wait_time: float = (next_card.fsrs_card.due - datetime.now(timezone.utc)).total_seconds()
    ...
    for my_card in due_cards:
        print("\n" + "="*50)
        print(f"Question: {my_card.front}")
        print(f"State: {my_card.fsrs_card.state.name}")
        
        input("\nPress Enter to show answer...")
        
        print(f"Answer: {my_card.back}")
        print("\n" + "="*50)
        ...
        # Review with FSRS
        updated_card: Card
        updated_card, log = scheduler.review_card(my_card.fsrs_card, rating)
        my_card.fsrs_card = updated_card
        
        save_user_cards(db, username, cards)
        reviewed += 1
        
        print(f"\nReviewed! Next due: {updated_card.due.strftime('%Y-%m-%d %H:%M')}")
        print(f"  Stability: {updated_card.stability:.2f} | Difficulty: {updated_card.difficulty:.2f}")
...
# STATISTICS
# ==========================
def show_stats(cards: list[MyCard]) -> None:
    """Show statistics"""
    if not cards:
        print("\nNo cards yet!")
        return
    ...
    learning: int = sum(1 for c in cards if c.fsrs_card.state in (State.Learning, State.Relearning))
...
# LIST CARDS
# ==========================
def list_cards(cards: list[MyCard]) -> None:
    """List all cards"""
    if not cards:
        print("\nNo cards yet!")
        return
    ...
    for i, my_card in enumerate(cards, 1):
        state: str = my_card.fsrs_card.state.name if not is_card_new(my_card.fsrs_card) else "New"
        print(f"{i}. {my_card.front} → {my_card.back} [{state}]")
...
def delete_user(db: dict) -> None:
    ...
    if choice.isdigit() and 1 <= int(choice) <= len(users):
        ...
        confirm: str = input(f"Delete user '{username}' and all cards? (yes/no): ").strip().lower()
...
def main() -> None:
    print("="*50)
    print("FLASHCARD APP (FSRS) - MULTI-USER")
    print("="*50)
    ...
    cards: list[MyCard] = load_user_cards(db, username)
