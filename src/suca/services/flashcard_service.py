...
class FlashcardService(BaseService[Flashcard]):
    ...
    def get_user_decks(self, user_id: str) -> DeckListResponse:
        ...
        try:
            statement = select(
                ...
                Flashcard, FlashcardDeck.id == Flashcard.deck_id
            ).where(
                FlashcardDeck.user_id == user_id
            ).group_by(
                FlashcardDeck.id
            ).order_by(
                FlashcardDeck.updated_at.desc()
            )
    ...
    def create_flashcard(self, user_id: str, flashcard_create: FlashcardCreate) -> FlashcardResponse:
    ...
    def update_flashcard(self, card_id: int, user_id: str, flashcard_update: FlashcardUpdate) -> FlashcardResponse:
        """Update a flashcard."""
        flashcard = self._get_flashcard_by_id(card_id, user_id)
