...
from ..db.model import Entry, Kanji, Reading, Sense, Gloss, Example
...
class SearchService(BaseService[Entry]):
    ...
    def _get_start_matches(self, query: str, include_rare: bool) -> List[Entry]:
        ...
        if not include_rare:
            ...
            kanji_stmt = select(Entry).join(Kanji).where(
                base_kanji_filter & (Entry.is_common == True)
            ).limit(5)
            results.extend(self.session.exec(kanji_stmt).all())
            
            reading_stmt = select(Entry).join(Reading).where(
                base_reading_filter & (Entry.is_common == True)
            ).limit(5)
